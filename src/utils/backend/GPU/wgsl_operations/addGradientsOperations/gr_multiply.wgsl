fn gr_multiply(gId : vec3u, currTensor : u32, childTensor: u32){
    ...
    let partner_rows = u32(ping.entries[u32(offset.tensor[curr_partner_id].rows)]);
    let partner_cols = u32(ping.entries[u32(offset.tensor[curr_partner_id].cols)]);
    ...
    if(isRightMultiplicator == u32(0)){
        ...
        let index = curr_GradientDataFirst + gId.x * curr_n + gId.y;
        ping.entries[index] = result / f32(childGradient_n);        
    } 
    else { // if(isRightMultiplicator == u32(1)){
