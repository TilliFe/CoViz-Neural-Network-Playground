...
export class Model {
  ...
  conv2D(left: Tensor, right: Tensor): Tensor {
    // right is the Kernel
    this.numTensors = this.numTensors + 1;
    const resRequiresGradient = true;
    const res = new Tensor(
      'conv2D',
      left.rows,
      left.cols,
  ...
  CE(trueValues: Tensor, predValues: Tensor): Tensor {
    ...
    this.numTensors = this.numTensors + 1;
    const resRequiresGradient = false; //  = trueValues.requiresGradient || predValues.requiresGradient;
